...
with events_with_ts as (
  ...
  select
    ...
    min(case when fe.event_type = 'enrollment' then fe.event_timestamp end)
      over(partition by fe.user_id, fe.course_id, fe.campaign_id) as min_enroll_ts,

    min(case when fe.event_type = 'view' then fe.event_timestamp end)
      over(partition by fe.user_id, fe.course_id, fe.campaign_id) as min_view_ts,

    min(case when fe.event_type = 'completion' then fe.event_timestamp end)
      over(partition by fe.user_id, fe.course_id, fe.campaign_id) as min_comp_ts,

    min(case when fe.event_type = 'feedback' then fe.event_timestamp end)
      over(partition by fe.user_id, fe.course_id, fe.campaign_id) as min_fb_ts
